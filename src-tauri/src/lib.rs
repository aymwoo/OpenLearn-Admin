...
fn get_system_info(state: State<'_, AppState>) -> Result<SystemInfo, String> {
    ...
    // Bolt Optimization: Only refresh necessary metrics instead of everything
    sys.refresh_cpu_usage();
    sys.refresh_memory();

    let mut disks = state.disks.lock().map_err(|e| format!("锁错误: {}", e))?;
    // Bolt Optimization: Cache Disks struct and refresh it instead of instantiating new lists
    disks.refresh(true);
