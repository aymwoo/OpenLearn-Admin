      ...
      <main className="flex-1 ml-64 flex flex-col h-screen overflow-hidden bg-background">
        <header className="flex justify-between items-center w-full px-8 py-4 backdrop-blur-xl bg-[#f7f9fb]/85 dark:bg-slate-950/85 shadow-[0_12px_40px_rgba(0,67,148,0.08)] z-10 sticky top-0">
          ...
          <div className="flex items-center space-x-6">
            <div className="flex items-center rounded-lg border border-blue-200 dark:border-blue-800 overflow-hidden">
              <div className="flex items-center justify-center px-3 py-2 bg-blue-100 dark:bg-blue-900/50">
                <span className={`material-symbols-outlined text-xl ${dbStatus?.connected ? 'text-emerald-500' : 'text-rose-500'}`}>
                  database
                </span>
              </div>
              <div className="flex items-center gap-1 px-2 py-2 bg-blue-50 dark:bg-blue-950/30">
                <button onClick={handleStartService} disabled={loading} title="启动服务" className="p-2 text-emerald-600 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 rounded">
                  <span className="material-symbols-outlined">play_arrow</span>
                </button>
                <button onClick={handleStopService} disabled={loading} title="停止服务" className="p-2 text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-900/30 rounded">
                  <span className="material-symbols-outlined">stop</span>
                </button>
                <button onClick={handleStartService} disabled={loading} title="重启服务" className="p-2 text-amber-600 hover:bg-amber-100 dark:hover:bg-amber-900/30 rounded">
                  <span className="material-symbols-outlined">refresh</span>
                </button>
              </div>
            </div>

            <div className="flex items-center rounded-lg border border-purple-200 dark:border-purple-800 overflow-hidden">
              <div className="flex items-center justify-center px-3 py-2 bg-purple-100 dark:bg-purple-900/50">
                <span className={`material-symbols-outlined text-xl ${wsConnectionError ? 'text-rose-500' : 'text-emerald-500'}`}>
                  language
                </span>
              </div>
              <div className="flex items-center gap-1 px-2 py-2 bg-purple-50 dark:bg-purple-950/30">
                <button onClick={handleStartService} disabled={loading} title="启动服务" className="p-2 text-emerald-600 hover:bg-emerald-100 dark:hover:bg-emerald-900/30 rounded">
                  <span className="material-symbols-outlined">play_arrow</span>
                </button>
                <button onClick={handleStopService} disabled={loading} title="停止服务" className="p-2 text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-900/30 rounded">
